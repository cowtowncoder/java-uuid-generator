    ...
    implements Serializable, Cloneable, Comparable
{
    ...
    public final static int INDEX_CLOCK_LO = 0;

    public final static int INDEX_TYPE = 6;
    // Clock seq. & variant are multiplexed...
    public final static int INDEX_CLOCK_SEQUENCE = 8;
    public final static int INDEX_VARIATION = 8;
    ...
    {
        mId = new byte[16];
        System.arraycopy(data, start, mId, 0, 16);
    }

    /**
     * Protected constructor used by UUIDGenerator.
     * Note that the byte array passed is considered to be owned by UUID
     * being constructed, and is specifically <b>not</b> copied to a new
     * byte array.
     *<p>
     *
     ...
     */
    protected UUID(int type, byte[] data)
    {
        // should we even test the length, or nullness?
        if (data.length != 16) {
    ...
    {
        System.arraycopy(mId, 0, dst, pos, 16);
    }
